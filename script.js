...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Event listeners with history pushes
  ...
  // Create a person node (with tighter name/surname spacing)
  function createPerson(id,name,father,surname,dob,birth,cx,cy,r,fill){
    ...
    g.addEventListener('touchend', e=>{
      ...
      if(len<500&&len>0){ pushHistory(); editPerson(e,c); e.preventDefault(); }
      else nodeClick(e,c);
      lastTap=now;
    });
  ...
  function applyChanges(){
    if(!selected) return;
    const g=selected._group, nc=colorPicker.value, nr=+sizeInput.value;
    ...
    // Reposition lines with new spacing
    selected._line1.setAttribute('x',cx);
    selected._line1.setAttribute('y',cy - r*0.25);
    selected._line2.setAttribute('x',cx);
    selected._line2.setAttribute('y',cy + r*0.05);
    selected._line3.setAttribute('x',cx);
    selected._line3.setAttribute('y',cy + r*0.5);
  }
  ...
  function drawLine(a,b){
    ...
    if(firstG) svg.insertBefore(l,firstG); else svg.appendChild(l);
  ...
  function setupDrag(c){
    ...
    function sD(e){
      ...
      off={ x:cx*(svg.clientWidth/viewBox.w)-start.x,
    ...
    function dD(e){
      ...
      c._line1.setAttribute('y',y - r*0.25);
      c._line2.setAttribute('x',x);
      c._line2.setAttribute('y',y + r*0.05);
      c._line3.setAttribute('x',x);
      c._line3.setAttribute('y',y + r*0.5);
  ...
  function editPerson(e,c){
    ...
    const full1=[g.getAttribute('data-name'),g.getAttribute('data-father-name')].filter(Boolean).join(' ');
    c._line1.textContent=full1;
    c._line2.textContent=g.getAttribute('data-surname');
    c._line3.textContent=g.getAttribute('data-dob');
  }
