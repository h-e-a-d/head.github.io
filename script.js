...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Add a person
  function addPerson() {
    const name = prompt('Enter given name:'); if (name === null) return;
  ...
  function createPerson(id, name, father, surname, dob, birth, cx, cy, r, fill) {
    ...
    c.setAttribute('stroke', '#2980b9');
    c.setAttribute('stroke-width', '2');
    c.setAttribute('class', 'person');

    const full = [name, father, surname].filter(Boolean).join(' ');
    const t1 = document.createElementNS(svg.namespaceURI, 'text');
    t1.setAttribute('x', cx); t1.setAttribute('y', cy - 10);
    t1.setAttribute('text-anchor', 'middle'); t1.setAttribute('class', 'name');
    t1.textContent = full;

    const t2 = document.createElementNS(svg.namespaceURI, 'text');
    t2.setAttribute('x', cx); t2.setAttribute('y', cy + 15);
    t2.setAttribute('text-anchor', 'middle'); t2.setAttribute('class', 'dob');
    t2.textContent = dob;

    g.appendChild(c); g.appendChild(t1); g.appendChild(t2);
    svg.appendChild(g);

    c._group = g; c._nameText = t1; c._dobText = t2;
  ...
  function setupDrag(c) {
    ...
    function startDrag(e) {
      e.stopPropagation();
      start = e.touches ? { x: e.touches[0].clientX, y: e.touches[0].clientY }
    ...
    function doDrag(e) {
      if (!start) return;
      e.preventDefault();
      const mv = e.touches ? { x: e.touches[0].clientX, y: e.touches[0].clientY }
                           : { x: e.clientX, y: e.clientY };
      let x = (mv.x + off.x) * (viewBox.w / svg.clientWidth);
      let y = (mv.y + off.y) * (viewBox.h / svg.clientHeight);
      x = Math.round(x / 10) * 10; y = Math.round(y / 10) * 10;
      c.setAttribute('cx', x); c.setAttribute('cy', y);
      c._nameText.setAttribute('x', x); c._nameText.setAttribute('y', y - 10);
      c._dobText.setAttribute('x', x); c._dobText.setAttribute('y', y + 15);
      if (c._group._lines) {
  ...
  function editPerson(e, c) {
    ...
    c._nameText.textContent = full;
    c._dobText.textContent  = g.getAttribute('data-dob');
  }

  // Debug connections (Ctrl+D)
  function debugConnections() {
    svg.querySelectorAll('line.relation').forEach((l, i) => {
      console.log(
        `Connection ${i+1}: ${l.getAttribute('data-source')} -> ${l.getAttribute('data-target')}`
      );
    });
  }
  ...
  // Helper: get current state for serialization
